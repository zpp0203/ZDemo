...
public class DrawLine implements Renderer
{
	...
	float x=0.6f,y=0.6f,z=0.6f;
	private float r=0;//旋转角度
	...
	void updateXYZ(){
		...
		lineColosBuffer = floatBufferUtil(lineColos);
...
//		timer.schedule(task, 0, 7000);
	...
	private void sendChangeToActivity(){
...
//			xyz[2]=String.valueOf(z).substring(0, 5);
		...
		xyz[0]=String.valueOf(x);
		xyz[1]=String.valueOf(y);
		xyz[2]=String.valueOf(z);
	...
	public void onDrawFrame(GL10 gl)
	{
		...
		// 重置当前的模型视图矩阵
		gl.glLoadIdentity();
		gl.glTranslatef(0f, -0f, -3.0f);//移动中心
		// 沿着Y轴旋转
		//gl.glRotatef(r, 0f, 0.1f, 0.0f);
		//r++;

		// 沿着X轴旋转
//		gl.glRotatef(15, 0.1f, 0f, 0f);
//		gl.glRotatef(-25, 0f, 0.1f, 0f);

		gl.glLineWidth(2.0f);
		// 设置顶点的位置数据 因为所有的数据都在次数组中，所以长方体和向量的只要设置这一次就好
		gl.glVertexPointer(3, GL_FLOAT, 0, lineVerticesBuffer);
		// 设置顶点的颜色数据
		gl.glColor4f(1.0f, 0.0f, 0.0f, 1.0f);
		// 颜色数组也需要启用
		//gl.glEnableClientState(GL_COLOR_ARRAY);
		//gl.glColorPointer(3, GL_FLOAT, 0, lineColosBuffer);
...
//		gl.glVertexPointer(3, GL10.GL_FLOAT, 0, lineVerticesBuffer);//向量
		...
		gl.glVertexPointer(3, GL_FLOAT, 0, xyzVerticesBuffer);
		// 设置顶点的颜色数据
